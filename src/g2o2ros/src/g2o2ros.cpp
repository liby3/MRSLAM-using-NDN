...
int main(int argc, char **argv) {
  ...
  float resolution, max_range, usable_range, angle, threshold, free_threshold;
  string g2oFilename, mapFilename;
  g2o::CommandArgs arg;
  arg.param("resolution", resolution, 0.05f, "resolution of the map (how much is in meters a pixel)");
  arg.param("threshold", threshold, .0f, "threshold to apply to the frequency map (values under the threshold are discarded)");
  ...
  arg.param("free_threshold", free_threshold, .0f, "Pixels with occupancy probability less than this threshold are considered completely free");
  ...
  for(int c = 0; c < map.cols(); c++) {
	...
	if (free_threshold && fraction < free_threshold)
	  mapImage.at<unsigned char>(r, c) = 254;
	else if (threshold && fraction > threshold)
...
cout << "saved map....." << endl;
  ...
  ofs << "image: " << mapFilename << ".png" << std::endl
      ...
      << "negate: 0" << std::endl
      << "occupied_thresh: " << threshold << std::endl
      << "free_thresh: " << free_threshold << std::endl;
  return 0;
}
